...
export default async function handler(req, res) {
  try {
    // Guard token to prevent public abuse
    ...
    for (const u of users) {
      ...
      if (createErr) {
        // If the user already exists, fetch the auth user by email
        // and continue to upsert the profile.
      ...
      // 2) Upsert into public.profiles (this assumes the `profiles` table exists)
      // If userId is null (user existed), try to resolve using Admin API list
      let idToUse = userId;
